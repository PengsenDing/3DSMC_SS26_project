...
This detects landmarks, fits camera/identity/expression, fits appearance
(albedo + illumination), fits per-vertex RGB color, exports the mesh, and (with
`--render`) renders the four basic viewer modes. Output goes to one
...
├── fitting.txt                solver report and RMSE
...
0. Python entry point
   ...
   steps 2-8, and (if --render is passed) invokes `landmark_viewer` for step
   9. Collects every output file into one reconstructions/<name>/ folder.
        ↓
...
3. Camera + identity + expression fitting (sparse landmark loss)
   ...
   fitted camera. Every step after this depends on this output.
        ↓
4. Rasterization (visibility test, used by the next two steps)
   face_recon/rasterizer.h / .cc
   tests/rasterizer_test.cpp
   Given the mesh and camera from step 3, this answers a purely geometric
   ...
   surface lookup table used by steps 5, 6, and 8.
        ↓
5. Appearance fitting (albedo + illumination separation)
   face_recon/photometric_fitting.h / .cc
   tests/photometric_fitting_test.cpp
   Using step 4's pixel-to-surface mapping, looks at the actual photo colors
   ...
   permanently baked into the estimated skin color.
        ↓
6. Dense per-vertex RGB fitting (final visual result)
   face_recon/texture_fitting.h / .cc
   tests/texture_fitting_test.cpp
   With geometry and visibility already fixed (steps 3-4), this solves
   ...
   toward the step-5 result), solved directly without Ceres. This is the
   ...
   real photo.
        ↓
7. Mesh export + fitting report
   face_recon/mesh_export.h / .cc
   Pure I/O -- computes nothing new. Writes the step-3 vertices plus
   whichever colors are available (step 5 or 6) to disk as .off (geometry
   only, for MeshLab) and .ply (geometry + color), and writes fitting.txt
   with the solver's RMSE numbers and fitted parameters.
        ↓
8. Diagnostic image export (overlay / depth / visibility)
   ...
   accuracy), raster_depth.png (visualized depth buffer, to check step 4),
   and visibility.png (visible-face mask, to check the silhouette). Only
   generated when --diagnostics is passed.
        ↓
9. (Optional, only runs with --render) Four rendering modes
   ...
   step-7 face.ply through a real OpenGL pipeline and renders it as
   albedo/depth/normal/checkerboard. It has no data dependency on steps 1-8
...
```text
tests/            All 4 regression tests referenced above; run with `ctest --test-dir build`
